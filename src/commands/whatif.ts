...
var quoteList = [
  ...
  ,"that there is a secret within this command..."
];

export const whatif: CommandInterface = {
  data: new SlashCommandBuilder()
    .setName("whatif")
    .setDescription("What if..."),
  run: async (interaction) => {
    const quote = quoteList[Math.floor(Math.random() * quoteList.length - 1)]; // picks a random website from the list
    await interaction.reply("What if I told you \n"+ quote); // ...and posts it.
  }
};
